...
app.MapPost("/personas", async (Persona persona, ApplicationDbContext context, IOutputCacheStore outputCacheStore) =>
{
    context.Add(persona);
...
app.MapPut("/personas/{id:int}", async Task<Results<BadRequest<string>,NotFound<string>, NoContent>>
    (int id, Persona persona, ApplicationDbContext context, IOutputCacheStore outputCacheStore) =>
{
    if (id != persona.Id)
    {
    ...
    if (!existe)
    {
        return TypedResults.NotFound("El registro a modificar no existe");
    }
...
app.MapDelete("/personas/{id:int}", async Task<Results<NotFound<string>, NoContent>>(int id, ApplicationDbContext context, IOutputCacheStore outputCacheStore) =>
{
    var registrosBorrados = await context.Personas.Where(p => p.Id == id).ExecuteDeleteAsync();

    if (registrosBorrados == 0)
    {
        return TypedResults.NotFound("El registro a eliminar no existe");
    }
